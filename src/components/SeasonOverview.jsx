...
const SeasonOverview = ({ setActiveTab }) => {
  ...
  <div className="absolute inset-0 z-0">
    ...
    />
    {/* Gradiente para suavizar la imagen y mejorar legibilidad */}
  ...
  <div className="space-y-8 relative z-10">
    ...
    {/* SCOREBOARD CON LIGERO GLASSMORPHISM */}
    <div className="bg-slate-900/95 backdrop-blur-sm border border-slate-800 rounded-sm overflow-hidden shadow-2xl">
      <div className={`flex items-center p-4 border-b border-slate-800 transition-colors ${lastMatch.is_win ? 'bg-emerald-600/90' : 'bg-transparent'}`}>
        <div className="flex items-center gap-3 flex-1 relative">
          ...
          {lastMatch.is_win && (
            <div className="absolute -top-6 -left-3 bg-white px-2 py-0.5 transform -skew-x-12 flex items-center gap-1.5 shadow-xl">
        ...
        <div className="flex gap-1.5">
          {scoreSets.map((set, i) => (
            <div key={i} className={`w-10 h-12 flex items-center justify-center font-black italic text-xl transform -skew-x-6 shadow-md ${lastMatch.is_win ? 'bg-white text-emerald-600' : 'bg-slate-800 text-slate-400'}`}>
              {set.split('-')[0]}
            </div>
          ))}
        </div>
      ...
      <div className={`flex items-center p-4 relative ${!lastMatch.is_win ? 'bg-emerald-600/90' : 'bg-transparent'}`}>
        <div className="flex items-center gap-3 flex-1 relative">
          ...
          <span className={`text-xl uppercase tracking-tight ${!lastMatch.is_win ? 'text-white font-black italic' : 'text-slate-400 font-bold'}`}>
    ...
    {lastMatch.stats && (
      <button 
        ...
        className="w-full bg-white/50 backdrop-blur-sm border-2 border-slate-200 py-3 flex items-center justify-center gap-3 group hover:bg-slate-900 hover:border-slate-900 transition-all duration-300 shadow-sm"
      >
  ...
  <footer className="mt-8 flex items-center justify-between pt-4 border-t border-slate-200 relative z-10">
    <div className="flex items-center gap-2 text-slate-500 font-black italic text-[10px] uppercase tracking-[0.2em]">
    ...
    <div className="bg-[#0a0c10] border border-white/10 w-full max-w-2xl rounded-xl shadow-[0_0_60px_rgba(0,0,0,0.8)] overflow-hidden relative flex flex-col max-h-[95vh]">
      ...
      <div className="bg-[#0a0c10]/80 backdrop-blur-sm px-5 py-3 flex justify-between items-center border-b border-white/5 relative z-10">
        ...
        <button 
          ...
          className="group flex items-center gap-1.5 text-slate-500 hover:text-rose-500 transition-colors"
        >
          <span className="font-black text-[9px] tracking-widest uppercase">Close</span>
          <div className="w-5 h-5 rounded-full bg-white/5 border border-white/10 flex items-center justify-center group-hover:border-rose-500/50 group-hover:bg-rose-500/10 transition-all">
      ...
      <div className="p-4 md:p-6 relative z-10 overflow-hidden flex flex-col">
        {/* VERSUS HEADER COMPACTO CON FONDO MÁS OSCURO PARA LEGIILIDAD */}
        <div className="flex items-center justify-between mb-6 bg-[#05070a]/90 backdrop-blur-sm p-4 rounded-lg border border-white/5 shadow-xl relative z-10">
          ...
          <div className="flex-1 text-right flex flex-col items-end">
            ...
            <div className="flex items-center justify-end gap-2 mt-2">
              {lastMatch.opponent_flag && (
                <img src={`https://flagcdn.com/w40/${lastMatch.opponent_flag.trim().toLowerCase()}.png`} className="h-3 w-auto rounded-sm opacity-90" alt="Flag" crossOrigin="anonymous" />
              )}
            </div>
          </div>
        </div>

        {/* TABLA DE STATS - FILAS MÁS OPACAS PARA LEER MEJOR SOBRE LA FOTO */}
        <div className="space-y-1.5 overflow-y-auto pr-1 max-h-[60vh] custom-scrollbar relative z-10">
          {Object.entries(selectedStats).map(([key, data]) => {
            ...
            let rublevWins = false;
            let oppWins = false;
            if (rublevVal !== oppVal) {
              if (isNegativeStat) {
                rublevWins = rublevVal < oppVal;
                oppWins = oppVal < rublevVal;
              } else {
                rublevWins = rublevVal > oppVal;
                oppWins = oppVal > rublevVal;
              }
      ...
      <div className="bg-[#05070a] p-3 flex justify-between items-center border-t border-white/5 relative z-20 mt-auto">
        ...
        <div className="flex items-center gap-1.5 opacity-40">
           ...
           <p className="text-[8px] font-black uppercase tracking-[0.3em] text-slate-600">ATP Official Records</p>
        </div>
      </div>
